...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        self.gan = args.gan_augmentation
        self.neg_samples_num = args.neg_samples_num  # Number of negatives to randomly sample = 1000
        self.negs_num_per_query = negs_num_per_query  # Number of negatives per query in each batch = 10
    ...
    def __getitem__(self, index):
        ...
        # query = self.adjust_query_transform(path_to_pil_img(self.queries_paths[query_index]))
...
class CustomBrightnessTramsforms:
    def __init__(self, brightness):
        self.brightness = brightness
    
    def __call__(self, img):
        img = transforms.functional.adjust_brightness(img, brightness_factor=self.brightness)
        return img

class CustomContrastTramsforms:
...
class CustomSaturationTramsforms:
    def __init__(self, saturation):
        self.saturation = saturation
    
    def __call__(self, img):
        img = transforms.functional.adjust_saturation(img, saturation_factor=self.saturation)
        return img

class CustomSharpnessTramsforms:
